...
msoa = st_read("NHS South East MSOAs.gpkg", quiet = T) %>% `st_crs<-`(27700)
...
tm_shape(msoa) + tm_polygons("Demand", palette = "Reds", n = 5, type = "jenks") +
  tm_layout(title = "Hospital Admission Demand", title.position = c("right", "top"),
...
tm_shape(supply) + tm_dots(col = "white") + 
  ...
  tm_layout(title = "New Hospital Sites",
            frame = T, legend.outside = F, title.position = c("right", "top"),
